...
summarizer_dir = './record/E2E_DDPG_rnn/nav{}'.format(maze_id)
...
critic_network_spec = [
    [
        ...
        dict(type='conv2d', size=64, window=(4, 3), stride=2, activation='relu', padding='SAME'),
        dict(type='pool2d', pooling_type='max', window=2, stride=2, padding='SAME'),
        dict(type='conv2d', size=128, window=2, stride=2, activation='relu', padding='SAME'),
        dict(type='flatten'),
        dict(type='output', name='image_output')
    ],
    [
        dict(type='input', names=['image_output', 'relative_pos', 'angular_vel', 'linear_vel'],
             aggregation_type='concat'),
        dict(type='dense', size=256, activation='relu'),
        dict(type='dense', size=128, activation='relu'),
        dict(type='output', name='output2')
    ],
    [
        dict(type='input', names=['output2', 'previous_act', 'previous_reward'], aggregation_type='concat'),
        dict(type='dense', size=64, activation='relu'),
        dict(type='dense', size=1, activation='tanh')
    ]
]
...
while True:
    ...
    if GazeboMaze.goal not in config.test_space[maze_id]:  # train
        ...
        if episode % 1000 == 0:
            f = open(record_dir + '/E2E_DDPG_rnn_nav{}'.format(maze_id) + '.txt', 'a+')
